...
VirtualMonitor::VirtualMonitor() :
    ...
    _modeAdoptedCallback(NULL),
    _modeAdoptedCallbackUserData(NULL)
{
...
bool VirtualMonitor::Create(int width, int height, int left, int top, int index, bool isPrimary) {
    ...
    // Allow at least 5K since a later dynamic resize may increase the resolution (cannot be changed after creation)
    int maxPixelsWide = (width > 5120) ? width : 5120;
    int maxPixelsHigh = (height > 2880) ? height : 2880;
    ...
    _virtualDisplayInfo[_virtualDisplayInfoCnt].max_width = maxPixelsWide;
    _virtualDisplayInfo[_virtualDisplayInfoCnt].max_height = maxPixelsHigh;
    ...
    _virtualDisplayInfo[_virtualDisplayInfoCnt].resolution_established = 0;
    _virtualDisplayInfo[_virtualDisplayInfoCnt].last_mode_width = 0;
    _virtualDisplayInfo[_virtualDisplayInfoCnt].last_mode_height = 0;
...
bool VirtualMonitor::Resize(int index, int width, int height, int left, int top, bool isPrimary) {
    ...
    if (index < 0 || index >= _virtualDisplayInfoCnt) return false;
    if (_virtualDisplayInfo[index].virtualDisplay == nil) return false;

    // An in-place resize beyond the creation-time pixel caps can never succeed;
    // fail fast so the caller recreates the display instead of spinning the retry loop
    if (width > _virtualDisplayInfo[index].max_width || height > _virtualDisplayInfo[index].max_height) {
        NSLog(@"[VirtualMonitor::Resize] %dx%d exceeds creation cap %dx%d. recreate required",
              width, height, _virtualDisplayInfo[index].max_width, _virtualDisplayInfo[index].max_height);
        return false;
    }

    // Hold the lock throughout so the watch thread does not revert to the previous resolution/layout
    pthread_mutex_lock(&_watchLock);

    struct VIRTUALMONITOR_INFO* displayInfo = &_virtualDisplayInfo[index];

    // Ignore if nothing changed
    if (displayInfo->width == width && displayInfo->height == height &&
        displayInfo->left == left && displayInfo->top == top &&
        displayInfo->is_primary == (isPrimary ? 1 : 0)) {
    ...
    // HiDPI mode above a certain resolution (same criteria as Create)
    displayInfo->is_retina = (width > 2300 && height > 1500) ? true : false;
    // New target resolution — let the watchdog converge to it again
    displayInfo->resolution_established = 0;
    displayInfo->last_mode_width = 0;
    displayInfo->last_mode_height = 0;

    CGVirtualDisplayMode* mode = [[CGVirtualDisplayMode alloc] initWithWidth:width height:height refreshRate:60];
    CGVirtualDisplayMode* retinaMode = [[CGVirtualDisplayMode alloc] initWithWidth:width / 2 height:height / 2 refreshRate:60];
    CGVirtualDisplaySettings* settings = [[CGVirtualDisplaySettings alloc] init];
    if (mode == nil || retinaMode == nil || settings == nil) {
        pthread_mutex_unlock(&_watchLock);
        return false;
    }

    settings.hiDPI = displayInfo->is_retina ? 1 : 0;

    // Same as Create: fill in the default modes like a real monitor and put the xrdp resolution last.
    settings.modes = @[
        [[CGVirtualDisplayMode alloc] initWithWidth:3840 height:2160 refreshRate:60],
        [[CGVirtualDisplayMode alloc] initWithWidth:2560 height:1440 refreshRate:60],
        [[CGVirtualDisplayMode alloc] initWithWidth:1920 height:1080 refreshRate:60],
    ...
    [displayInfo->virtualDisplay applySettings:settings];
    ...
    // Display reconfiguration is asynchronous, so wait until it takes effect (retry periodically)
    bool resolutionOk = false;
    for (int i = 0; i < 20; i++) {
        if (IsRightResolution(index) == true) {
            resolutionOk = true;
            break;
        }
    ...
    NSLog(@"[VirtualMonitor::Resize] index=%d %dx%d (%d,%d) primary=%d result=%d",
          index, width, height, left, top, isPrimary ? 1 : 0, resolutionOk ? 1 : 0);
...
void VirtualMonitor::SetModeAdoptedCallback(on_display_mode_adopted cb, void* userData) {
    _modeAdoptedCallback = cb;
    _modeAdoptedCallbackUserData = userData;
}
...
int VirtualMonitor::ApplyDisplayLayout() {
    ...
    // Display origins were (re)applied — the cached input layout may be stale,
    // so ask for it to be rebuilt from live bounds (flag-only, refresh is idempotent)
...
void VirtualMonitor::WatchThreadPorcInternal() {
    ...
    // If a virtual display went offline (sleep/wake, re-registration), re-arm
    // resolution enforcement so the target mode is restored when it comes back —
    // a mode reset in that window is drift, not a user choice
    if (IsAllVirtualDisplayOnline() == false) {
        for (int i = 0; i < _virtualDisplayInfoCnt; i++) {
            _virtualDisplayInfo[i].resolution_established = 0;
        }
    ...
    for (int i = 0; i < _virtualDisplayInfoCnt; i++) {
        ...
        if (displayInfo->resolution_established == 0) {
        ...
        // The target resolution was reached before but the mode differs now:
        // the user changed the display mode externally (e.g. scaling option in
        // System Settings). Adopt it instead of reverting.
        if (displayInfo->virtualDisplay == nil) {
            continue;
        }
        ...
        if (modeWidth != displayInfo->last_mode_width || modeHeight != displayInfo->last_mode_height) {
            displayInfo->last_mode_width = modeWidth;
            displayInfo->last_mode_height = modeHeight;

            NSLog(@"[VirtualMonitor::WatchThreadProc] user selected display mode adopted. index=%d mode=%dx%d", i, modeWidth, modeHeight);
