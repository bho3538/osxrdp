...
void PaintRFX::DoPaint(const struct mod* mod, screenrecord_frame_t* frameInfo, char* imgData, size_t imgDataSize, int frame_id) {
    ...
    *(int*)((char*)_drawCmd->data_start + sizeof(int)) = dataLen;
    ...
    // RFX progressive 데이타
    // 입력 포맷은 SHM에 저장된 packed YUV444(Cr,Y,Cb, v308)이다.
    // 각 타일은 64x64 크기의 Y/U/V/A plane 순서로 채운다.
    ...
    memset(mapped, 0x00, _tileDataSize);
        ...
        for (int py = 0; py < 64; py++) {
            ...
            for (int px = 0; px < 64; px++) {
                ...
                if (sx < _width && sy < _height) {
                    const size_t srcOffset = ((size_t)sy * (size_t)_srcStride) + ((size_t)sx * 3);
                    vPlane[p] = src[srcOffset];
                    yPlane[p] = src[srcOffset + 1];
                    uPlane[p] = src[srcOffset + 2];
                }
                else {
                    yPlane[p] = 0;
                    uPlane[p] = 128;
