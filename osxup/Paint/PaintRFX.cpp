...
void PaintRFX::DoPaint(const struct mod* mod, screenrecord_frame_t* frameInfo, char* imgData, size_t imgDataSize, int frame_id) {
    ...
    *(int*)((char*)_drawCmd->data_start + sizeof(int)) = dataLen;
    ...
    // RFX progressive 데이타
    if (imgDataSize < _srcMinSize || _tileDataSize == 0) {
        return;
    }
    ...
    // MAP_ANON은 이미 0으로 초기화된 메모리를 반환하므로 전체 버퍼 memset 불필요
    ...
    for (int i = 0; i < tileCount; ++i) {
        ...
        unsigned char* tileBase = dst + ((size_t)tileIdx * 64 * 64 * 4);
        ...
        const int validWidth = (_width - left < 64) ? (_width - left) : 64;
        const int validHeight = (_height - top < 64) ? (_height - top) : 64;
        
        for (int py = 0; py < 64; py++) {
            const size_t pBase = (size_t)py * 64;
            
            if (py < validHeight) {
                const int sy = top + py;
                const unsigned char* srcRow = src + ((size_t)sy * (size_t)_srcStride) + ((size_t)left * 3);
                
                for (int px = 0; px < validWidth; px++) {
                    vPlane[pBase + px] = srcRow[0];
                    yPlane[pBase + px] = srcRow[1];
                    uPlane[pBase + px] = srcRow[2];
                    srcRow += 3;
                }
                
                // 가로 패딩 구간
                for (int px = validWidth; px < 64; px++) {
                    yPlane[pBase + px] = 0;
                    uPlane[pBase + px] = 128;
                    vPlane[pBase + px] = 128;
                }
